...
public class Resultset implements ResultSet {
	...
	public boolean next() throws SQLException {
		if (isClosed || currentRow >= data.size())
	...
	public String getString(int columnIndex) throws SQLException {
		columnIndex--;
		if (isClosed || currentRow < 0 || currentRow >= data.size())
			throw new SQLException();
		if (columnIndex < 0 || columnIndex >= data.get(currentRow).size())
			throw new SQLException();
		if (data.get(currentRow).get(columnIndex) == null)
			return null;
	...
	public boolean absolute(int row) throws SQLException {
		...
		if (row > data.size() || row + data.size() + 1 < -1) {
			...
			if (row > data.size())
				currentRow = data.size();
			else if (row + data.size() + 1 < -1)
				currentRow = -1;
		} else if (row < -1)
			currentRow = row + data.size() + 1;
		else
			currentRow = row;
		if (flag == 1 || currentRow == -1 || currentRow == data.size())
			return false;
		else
			return true;
	}
	...
	public java.sql.Array getArray(int columnIndex) throws SQLException {
		...
		if (columnIndex < 0 || columnIndex >= data.get(currentRow).size())
			throw new SQLException();
		return (java.sql.Array) data.get(currentRow).get(columnIndex);
	}
